...
Description:    This file aims to train and validate specified model. This process could be repeated for chosen
                number of epochs. You can choose, what process you want to do:
                    ...
                    > find learning rate - this function runs process of searching for optimal learning rate of chosen
                    model.
"""
...
def find_lr(trn_loader, net, optimizer, criterion, device, init_value=1e-12, final_value=10., beta=0.98):
    ...
    for data in trn_loader:
        ...
        # plot loss after one episode of searching
        print('Actual lr: {:.9f}\t Actual loss: {:.4f}'.format(lr, smoothed_loss))

    plt.plot(log_lrs[10:-5], losses[10:-5], "b-")
    plt.xlabel("Learning rate")
    plt.ylabel("Loss")
    plt.title("Learning rate searcher for" + cfg.hyperparameter.model_title)
    plt.savefig(cfg.path.graphs + f'lr_searcher' + cfg.hyperparameter.saving_string + '.png', format='png')
    plt.show()
...
def find_extreme_examples():
    ...
    lowest_loss = np.empty(number_of_examples); lowest_loss.fill(math.inf)
    lowest_loss_patch = np.empty((number_of_examples, cfg.hyperparameter.patch_size, cfg.hyperparameter.patch_size, 3), dtype=float)
    lowest_loss_mask = np.empty((number_of_examples, cfg.hyperparameter.patch_size, cfg.hyperparameter.patch_size), dtype=float)
    lowest_loss_prediction = np.empty((number_of_examples, cfg.hyperparameter.patch_size, cfg.hyperparameter.patch_size), dtype=float)

    highest_loss = np.zeros(number_of_examples)
    highest_loss_patch = np.empty((number_of_examples, cfg.hyperparameter.patch_size, cfg.hyperparameter.patch_size, 3), dtype=float)
    highest_loss_mask = np.empty((number_of_examples, cfg.hyperparameter.patch_size, cfg.hyperparameter.patch_size), dtype=float)
    highest_loss_prediction = np.empty((number_of_examples, cfg.hyperparameter.patch_size, cfg.hyperparameter.patch_size), dtype=float)
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(val_loader):
            ...
            indices = [i for i, v in enumerate(lowest_loss) if v > loss]
            if indices:
                lowest_loss[indices[0]] = loss
                patch = 255 * patches[0].cpu().numpy()
                patch = np.transpose(patch, (1, 2, 0))
                lowest_loss_patch[indices[0]] = patch
                lowest_loss_mask[indices[0]] = 255 * masks[0].cpu().numpy()
                lowest_loss_prediction[indices[0]] = 255 * outputs[0][1].cpu().numpy()

            indices = [i for i, v in enumerate(highest_loss) if v < loss]
            if indices:
                highest_loss[indices[0]] = loss
                patch = 255 * patches[0].cpu().numpy()
                patch = np.transpose(patch, (1, 2, 0))
                highest_loss_patch[indices[0]] = patch
                highest_loss_mask[indices[0]] = 255 * masks[0].cpu().numpy()
                highest_loss_prediction[indices[0]] = 255 * outputs[0][1].cpu().numpy()
...
def process_of_evaluation():
    ...
    if cfg.hyperparameter.save_run:
        plot_utils.plot_confusion_matrix(cm=confusion_matrix, normalize=False, title="Confusion matrix", cmap=None, target_names=cfg.hyperparameter.classes)
...
if __name__ == "__main__":
    ...
    if args.type == 'find_lr':
        ...
        find_lr(train_loader, model, optimizer, criterion, device)
        print("Process of searching learning rate is done!")

    # storing extreme examples (with high and low loss) process
    if args.type == 'find_extreme':
        print("Starting process of storing extreme examples...")
        find_extreme_examples()
        print("Process of storing extreme examples is done!")
